...
function installNodejs() 
{
    ...
    # Install amd use the latest LTS version
...
function installGo() 
{
    ...
    then
        ...
        then
            ...
            if [ $? != 0 ]; then
                echo
                echo -e "${bold_Red}* Failed to send fail notification to core-server - # downloading GO #${NC}"
            fi
        ...
        then
            ...
            if [ $? != 0 ]; then
                echo
                echo -e "${bold_Red}* Failed to send fail notification to core-server - # installing GO #${NC}"
            fi
...
function installDocker() 
{
    ...
    then 
        ...
        if [ $? != 0 ]; then
            echo
            echo -e "${bold_Red}* Failed to send fail notification to core-server - # installing Docker #${NC}"
        fi
...
function installDockerCompose() 
{
    ...
    then 
        ...
        if [ $? != 0 ]; then
            echo
            echo -e "${bold_Red}* Failed to send fail notification to core-server - # installing Docker-Compose #${NC}"
        fi
